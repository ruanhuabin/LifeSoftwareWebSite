		...
		<div style="width: 100%;">
			...
			<div style="float:right; width:82%">
				...
				<table border="1">
					...
					<s:iterator value="#request.allL2Categories" id="category">

						<tr>
							<td><input type="text"
								style="border: none;background: transparent;" size="64"
								id='<s:property value="#category.parentCategoryName"/>'
								name="parentCategoryName"
								value='<s:property value="#category.parentCategoryName"/>'> </input>
							</td>
							<td><input type="text"
								style="border: none;background: transparent;" size="64"
								id='<s:property value="#category.categoryName"/>'
								name="categoryName"
								value='<s:property value="#category.categoryName"/>'> </input>
							</td>

							<td><a
								href='<%=basePath%>post/Admin_editSubCategory.action?scid=<s:property value="#category.scid"/>&pageNum=<s:property value="#request.subCategoryPageData.currentPage"/>&parentCategoryName=<s:property value="#category.parentCategoryName"/>&categoryName=<s:property value="#category.categoryName"/>'
								target="_blank">Edit</a>
							</td>
							<td><a
								href='<%=basePath%>post/Admin_deleteSubCategory.action?scid=<s:property value="#category.scid"/>&pageNum=<s:property value="#request.subCategoryPageData.currentPage"/>'>Delete</a>
							</td>
				...
				<s:if test='#request.updateSubCategoryInfoSource.equals("from_update")'> 			 
	            	<p><s:property value="#request.updateSubCategoryResult"/></p>
	        	</s:if>
...
<s:set var="currPageData" value="subCategoryPageData" scope="request"/>
<s:include value="npj_new.jsp">
	<s:param name="actionURL">Admin_getSubCategoryPreNextPageData.action</s:param>
</s:include>
